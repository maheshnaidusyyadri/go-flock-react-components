import { b as i, g as Z } from "./index-B7VMIACg.js";
/*!
...
const j = 540, z = (r) => document.querySelector(`${r}.ion-cloned-element`), I = (r) => r.shadowRoot || r, P = (r) => {
  ...
  for (const t of n) {
    const g = t.closest("ion-header"), e = g && !g.classList.contains("header-collapse-condense-inactive"), C = t.querySelector("ion-back-button"), s = t.classList.contains("buttons-collapse"), S = t.slot === "start" || t.slot === "";
    if (C !== null && S && (s && e && l || !s))
      return C;
  }
  return null;
}, J = (r, l, c, n, t) => {
  const g = D(n, c), e = P(t), C = P(n), s = D(t, c), S = g !== null && e !== null && !c, y = C !== null && s !== null && c;
  if (S) {
    ...
    const E = C.getBoundingClientRect(), m = s.getBoundingClientRect(), d = I(s).querySelector(".button-text"), R = d == null ? void 0 : d.getBoundingClientRect(), A = I(C).querySelector(".toolbar-title").getBoundingClientRect();
    G(r, l, c, C, E, A, m, d, R), k(r, l, c, s, m, d, R, C, A);
  }
  return {
    forward: S,
    backward: y
  ...
  if (g && e) {
    ...
    L = s.width / e.width, A = (s.height - U) / e.height, T = Y ? `scale(${L}, ${A})` : `scale(${A})`;
  }
  const x = I(n).querySelector("ion-icon").getBoundingClientRect(), W = l ? `${x.width / 2 - (x.right - t.right)}px` : `${t.left - x.width / 2}px`, o = l ? `-${window.innerWidth - t.right}px` : `${t.left}px`, p = `${s.top}px`, $ = `${t.top}px`, v = [
  ...
  let K = `scale(${T})`;
  if (C && s) {
    A = l ? `-${window.innerWidth - s.right - L}px` : `${s.x - L}px`;
    const N = ((S = C.textContent) === null || S === void 0 ? void 0 : S.trim()) === ((y = n.textContent) === null || y === void 0 ? void 0 : y.trim()), h = s.width / g.width;
    T = s.height / (g.height - U), K = N ? `scale(${h}, ${T})` : `scale(${T})`;
  }
  ...
  ], v = c ? p : $, a = z("ion-title"), f = i();
  a.innerText = n.innerText, a.size = n.size, a.color = n.color, f.addElement(a), f.beforeStyles({
    ...
    n.style.setProperty("opacity", ""), a.style.setProperty("display", "none");
  }).keyframes(v), r.addAnimation(f);
}, V = (r, l) => {
  var c;
  try {
    const n = "cubic-bezier(0.32,0.72,0,1)", t = "opacity", g = "transform", e = "0%", s = r.ownerDocument.dir === "rtl", S = s ? "-99.5%" : "99.5%", y = s ? "33%" : "-33%", E = l.enteringEl, m = l.leavingEl, d = l.direction === "back", R = E.querySelector(":scope > ion-content"), L = E.querySelectorAll(":scope > ion-header > *:not(ion-toolbar), :scope > ion-footer > *"), A = E.querySelectorAll(":scope > ion-header > ion-toolbar"), T = i(), X = i();
    ...
    if (!R && A.length === 0 && L.length === 0 ? X.addElement(E.querySelector(":scope > .ion-page, :scope > ion-nav, :scope > ion-tabs")) : (X.addElement(R), X.addElement(L)), T.addAnimation(X), d ? X.beforeClearStyles([t]).fromTo("transform", `translateX(${y})`, `translateX(${e})`).fromTo(t, 0.8, 1) : X.beforeClearStyles([t]).fromTo("transform", `translateX(${S})`, `translateX(${e})`), R) {
      ...
      if (o) {
        const p = o.querySelector(".transition-cover"), $ = o.querySelector(".transition-shadow"), v = i(), a = i(), f = i();
        v.addElement(o).beforeStyles({ opacity: "1", display: "block" }).afterStyles({ opacity: "", display: "" }), a.addElement(p).beforeClearStyles([t]).fromTo(t, 0, 0.1), f.addElement($).beforeClearStyles([t]).fromTo(t, 0.03, 0.7), v.addAnimation([a, f]), X.addAnimation([v]);
      }
    }
    const K = E.querySelector("ion-header.header-collapse-condense"), { forward: x, backward: W } = J(T, s, d, E, m);
    if (A.forEach((o) => {
      const p = i();
      p.addElement(o), T.addAnimation(p);
      const $ = i();
      $.addElement(o.querySelector("ion-title"));
      const v = i(), a = Array.from(o.querySelectorAll("ion-buttons,[menuToggle]")), f = o.closest("ion-header"), N = f == null ? void 0 : f.classList.contains("header-collapse-condense-inactive");
      let h;
      d ? h = a.filter((b) => {
        const F = b.classList.contains("buttons-collapse");
        return F && !N || !F;
      }) : h = a.filter((b) => !b.classList.contains("buttons-collapse")), v.addElement(h);
      const q = i();
      ...
      else if (K || $.fromTo("transform", `translateX(${S})`, `translateX(${e})`).fromTo(t, 0.01, 1), q.fromTo("transform", `translateX(${S})`, `translateX(${e})`), _.beforeClearStyles([t, "transform"]), (f == null ? void 0 : f.translucent) ? _.fromTo("transform", s ? "translateX(-100%)" : "translateX(100%)", "translateX(0px)") : _.fromTo(t, 0.01, "var(--opacity)"), x || O.fromTo(t, 0.01, 1), w && !x) {
        const F = i();
        F.addElement(I(w).querySelector(".button-text")).fromTo("transform", s ? "translateX(-100px)" : "translateX(100px)", "translateX(0px)"), p.addAnimation(F);
      }
    }), m) {
      ...
      if (!p && $.length === 0 && v.length === 0 ? o.addElement(m.querySelector(":scope > .ion-page, :scope > ion-nav, :scope > ion-tabs")) : (o.addElement(p), o.addElement(v)), T.addAnimation(o), d) {
        o.beforeClearStyles([t]).fromTo("transform", `translateX(${e})`, s ? "translateX(-100%)" : "translateX(100%)");
        const a = Z(m);
        T.afterAddWrite(() => {
          T.getDirection() === "normal" && a.style.setProperty("display", "none");
        });
      } else
        o.fromTo("transform", `translateX(${e})`, `translateX(${y})`).fromTo(t, 1, 0.8);
      if (p) {
        const a = I(p).querySelector(".transition-effect");
        if (a) {
          const f = a.querySelector(".transition-cover"), N = a.querySelector(".transition-shadow"), h = i(), q = i(), _ = i();
          h.addElement(a).beforeStyles({ opacity: "1", display: "block" }).afterStyles({ opacity: "", display: "" }), q.addElement(f).beforeClearStyles([t]).fromTo(t, 0.1, 0), _.addElement(N).beforeClearStyles([t]).fromTo(t, 0.7, 0.03), h.addAnimation([q, _]), o.addAnimation([h]);
        }
      }
      $.forEach((a) => {
        const f = i();
        f.addElement(a);
        const N = i();
        N.addElement(a.querySelector("ion-title"));
        const h = i(), q = a.querySelectorAll("ion-buttons,[menuToggle]"), _ = a.closest("ion-header"), O = _ == null ? void 0 : _.classList.contains("header-collapse-condense-inactive"), w = Array.from(q).filter((H) => {
          const Y = H.classList.contains("buttons-collapse");
          return Y && !O || !Y;
        });
        h.addElement(w);
        const b = i(), F = a.querySelectorAll(":scope > *:not(ion-title):not(ion-buttons):not([menuToggle])");
        F.length > 0 && b.addElement(F);
        const B = i();
        B.addElement(I(a).querySelector(".toolbar-background"));
        const u = i(), M = a.querySelector("ion-back-button");
        if (M && u.addElement(M), f.addAnimation([
          ...
          if (O || N.fromTo("transform", `translateX(${e})`, s ? "translateX(-100%)" : "translateX(100%)").fromTo(t, 0.99, 0), b.fromTo("transform", `translateX(${e})`, s ? "translateX(-100%)" : "translateX(100%)"), B.beforeClearStyles([t, "transform"]), (_ == null ? void 0 : _.translucent) ? B.fromTo("transform", "translateX(0px)", s ? "translateX(-100%)" : "translateX(100%)") : B.fromTo(t, "var(--opacity)", 0), M && !W) {
            const Y = i();
            Y.addElement(I(M).querySelector(".button-text")).fromTo("transform", `translateX(${e})`, `translateX(${(s ? -124 : 124) + "px"})`), f.addAnimation(Y);
          }
        } else
...
//# sourceMappingURL=ios.transition-BcSJqnjN.js.map
